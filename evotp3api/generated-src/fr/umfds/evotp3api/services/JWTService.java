...
public class JWTService {
    ...
    public String issue(User user) {
        return JWT.create().withIssuer(getIssuer()).withSubject("" + user.getId()).withClaim("username", user.getEmail()).withIssuedAt(Instant.now()).withExpiresAt(Instant.now().plus(31, ChronoUnit.DAYS)).withJWTId(UUID.randomUUID().toString()).sign(algorithm);
    }
    ...
    public Optional<String> extractUsername(String jwt) {
        try {
            return Optional.of(JWT.require(algorithm).withIssuer(getIssuer()).build().verify(jwt).getClaim("username").asString());
        } catch (Exception e) {
            e.printStackTrace();
        }
