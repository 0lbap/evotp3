...
public class LoggingInjector extends AbstractProcessor<CtClass<?>> {
    ...
    private void addLoggerField(CtClass<?> ctClass) {
        ...
        loggerField.setAssignment(getFactory().createCodeSnippetExpression("LoggerFactory.getLogger(\"fr.umfds.evotp3api.profiling\")"));
    ...
    private void injectLoggingWithThreadContext(CtClass<?> ctClass) {
        for (CtMethod<?> method : ctClass.getMethods()) {
            ...
            if (action != null) {
                ...
                StringBuilder threadContextSnippet = new StringBuilder().append("ThreadContext.put(\"user_id\", String.valueOf(userDetails.getId()));\n").append("ThreadContext.put(\"action\", \"").append(action).append("\")");
                ...
                if ("get".equals(methodName)) {
                    ...
                    method.getBody().getElements(e -> e instanceof CtReturn<?>).forEach(returnStmt -> {
                        // Cast to CtReturn
                        CtReturn<?> ctReturn = ((CtReturn<?>) (returnStmt));
                        // Inject ThreadContext snippet
                        CtCodeSnippetStatement threadContextSetup = getFactory().createCodeSnippetStatement(threadContextSnippet.toString());
    ...
    private void ensureAuthenticationPrincipalParameter(CtMethod<?> method) {
        ...
        boolean hasUserDetails = method.getParameters().stream().anyMatch(param -> "userDetails".equals(param.getSimpleName()));
        if (!hasUserDetails) {
            ...
            CtAnnotation<?> authenticationPrincipalAnnotation = getFactory().createAnnotation(getFactory().createCtTypeReference(AuthenticationPrincipal.class));
